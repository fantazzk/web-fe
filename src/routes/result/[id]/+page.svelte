...
<div class="flex h-screen flex-col bg-bg-primary">
	...
	<div bind:this={cardEl} class="flex flex-1 flex-col overflow-hidden">
		...
		<header class="flex flex-col items-center gap-2 px-8 pt-8 pb-4">
			...
			<span class="font-mono text-sm text-subtle">
				{tournament.date}
			</span>
		</header>
		...
		<!-- Team Cards -->
		<main class="flex flex-1 gap-4 overflow-hidden px-8 py-6">
			{#each teams as team, i (i)}
				<div class="flex flex-1 flex-col border border-gray-700">
					<!-- Captain Header -->
					<div class="flex flex-col items-center gap-1 bg-accent px-4 py-4">
						<span class="font-heading text-xl font-bold text-bg-primary">
							{team.captain}
						</span>
						<span
					...
					<!-- Player List -->
					<div class="flex flex-1 flex-col gap-4 px-4 py-5">
					...
					>
						<span class="font-mono text-xs font-semibold tracking-wider text-subtle"> TOTAL </span>
	...
	<div class="flex items-center justify-center gap-4 border-t border-gray-700 px-8 py-5">
		<Button variant="SECONDARY" size="MD" onclick={saveImage}>
		...
		<Button variant="PRIMARY" size="MD">다시 하기</Button>
	</div>
</div>
